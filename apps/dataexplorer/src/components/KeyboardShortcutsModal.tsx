...
function KeyCap({ children, disabled }: { children: React.ReactNode; disabled?: boolean }) {
  return (
    <kbd
      className={`inline-flex min-w-[26px] items-center justify-center rounded-md border border-border bg-muted/50 px-1.5 py-0.5 font-medium font-mono text-[11px] shadow-[0_1px_0_0_hsl(var(--border))] ${disabled ? 'text-muted-foreground/40' : 'text-muted-foreground'}
      `}
    >
      {children}
    </kbd>
      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        {groupedShortcuts.size > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-4 text-center">
            ...
            <Button variant="link" size="sm" className="mt-1" onClick={handleOpenSettings}>
              {t('settings.enableInSettings')}
            </Button>
          </div>
        )}
