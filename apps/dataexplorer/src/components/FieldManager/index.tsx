    ...
    <>
      ...
      <div className="flex shrink-0 items-center justify-between gap-2 border-b px-3 py-2">
        <div className="flex items-center gap-1.5">
          <span className={cn('font-medium text-sm', mobile && 'hidden')}>
            {t('fieldManager.title')}
          </span>
        </div>
        <div
          className={cn('flex items-center gap-0.5 rounded-sm border p-px', mobile ? 'h-9' : 'h-6')}
        >
          <button
            type="button"
            onClick={() => setView('table')}
            className={cn(
              'flex items-center gap-1 rounded-sm px-1.5 text-[11px] transition-colors',
              mobile ? 'h-8 px-2.5 text-xs' : 'h-5',
              view === 'table'
      ...
      <div
        className={cn(
          'grid',
          mobile ? 'grid-cols-1 divide-y overflow-y-auto' : 'grid-cols-2 divide-x'
        )}
      >
        {/* Left: attribute browser */}
        <div className={cn('flex flex-col', mobile ? 'h-[46vh]' : 'h-105')}>
          {/* Search */}
          <div className="border-b p-2">
            <div className="relative">
              ...
              <Input
                ...
                className={cn('pl-7 text-xs', mobile ? 'h-9 text-sm' : 'h-6')}
              />
            </div>
          </div>
                  ...
                  if (toOne) {
                    return (
                      <button
                        ...
                        className={cn(
                          'flex w-full items-center gap-1.5 rounded-md px-1.5 text-left hover:bg-muted/60',
                          mobile ? 'py-2.5' : 'py-1'
                        )}
                      >
      ...
      <div
        className={cn(
          ...
          mobile && 'flex-col items-stretch gap-2 pb-[max(0.75rem,var(--app-safe-bottom))]'
        )}
      >
        ...
        <Button
          ...
          size="sm"
          className={cn('gap-1.5', mobile ? 'h-11' : 'h-7')}
